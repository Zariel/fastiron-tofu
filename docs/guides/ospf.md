...
The current RESTCONF implementation manages area existence and interface bindings. Process settings, stub/NSSA options, redistribution, interface timers, authentication, and network type are not yet managed. Hardware validation on FastIron `09.0.10kT213` covers area and VE binding creation, imports, single/all-area discovery, CLI binding drift repair, area replacement, deletion, empty plans, and persistence across reboot. Serial configuration checks verified preservation of unrelated settings and exact cleanup. Ethernet and LAG binding compatibility has not yet been tested.
