...
On the tested router image, default VLAN 1 required an explicit mode to enable snooping: removing its override produced `no snoop : no local config` even with global active/version 3 configured. Explicit active/version 3 and passive/version 2 worked. Reset and deletion preserved the VLAN's other settings and global policy, with no-change plans afterward; cleanup restored running and startup configuration exactly. This is native behavior; the provider does not synthesize a local override when a field is omitted. Null query fields mean no VLAN override, not proof of enabled snooping.

Default VLAN 4095 is also supported. Hardware checks covered selecting it with `fastiron_default_vlan`, configuring active/version 3, querying its overrides, deleting the IGMP resource while retaining VLAN 4095, and resetting the default VLAN to 1. The default VLAN and VE properties were preserved, refreshed inventory and no-change plans matched native configuration, and cleanup restored running and startup configuration exactly.
