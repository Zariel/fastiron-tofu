...
Speed, duplex and clock writes are not exposed. Individual RESTCONF resets did not reliably restore automatic speed, while deleting the Ethernet container changed an unrelated DHCP-client setting. These operations require further transport support; the provider does not delete and reconstruct neighboring configuration to work around that boundary. Storm control, jumbo frames and DSCP trust are also not yet managed. [Protected-port isolation](protected-ports.md) has a separate resource for Ethernet and LAG interfaces.
