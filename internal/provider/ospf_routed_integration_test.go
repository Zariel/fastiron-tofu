...
resource "fastiron_router_ospf_interface" "test" {
 for_each = toset(["ethernet 1/1/9", "lag 59"])
...
`
	...
	check(map[string][]string{"0.0.0.0": {"ve 5"}, "0.0.0.59": {"ethernet 1/1/9", "lag 59"}})
	if output := strings.TrimSpace(run(0, "output", "-json", "area")); output != `{"area_id":"0.0.0.59","id":"0.0.0.59","interfaces":["ethernet 1/1/9","lag 59"]}` {
		t.Fatalf("area=%s", output)
	}
	run(0, "plan", "-detailed-exitcode", "-no-color")

	write("main.tf", base+strings.Replace(configuration, `toset(["ethernet 1/1/9", "lag 59"])`, `toset(["lag 59"])`, 1))
	run(0, "apply", "-auto-approve", "-no-color")
	check(map[string][]string{"0.0.0.0": {"ve 5"}, "0.0.0.59": {"lag 59"}})
	run(0, "plan", "-detailed-exitcode", "-no-color")
