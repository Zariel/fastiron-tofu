...
func applyMAC(ctx context.Context, device *fastiron.Device, desired macConfig) (*macConfig, error) {
	...
	return fastiron.Reconcile(ctx, device, func(update *fastiron.Update) (*macConfig, error) {
...
func deleteMAC(ctx context.Context, device *fastiron.Device, name string) error {
	...
	return device.Update(ctx, func(update *fastiron.Update) error {
		...
		writeErr := update.DeleteIfPresent(macPath(name))
