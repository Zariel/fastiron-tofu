...
func TestNativeIP(t *testing.T) {
	current, unowned, err := nativeIP(configtest.Parse(t, nativeFixture(`ver 09.0.10kT213
...
func TestNativeIPOwnership(t *testing.T) {
	for name, rule := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			if _, _, err := nativeIP(configtest.Parse(t, nativeFixture("ip access-list extended EDGE\n "+rule+"\nend")), ipv4ACL, "EDGE"); err == nil {
...
func TestNativeIPPresence(t *testing.T) {
	empty, _, err := nativeIP(configtest.Parse(t, nativeFixture("ip access-list extended EDGE\nend")), ipv4ACL, "EDGE")
	if err != nil || empty == nil || len(empty.Rules) != 0 {
		t.Fatalf("empty ACL = %+v, %v", empty, err)
	}
	absent, _, err := nativeIP(configtest.Parse(t, nativeFixture("ipv6 access-list EDGE\n sequence 10 permit ipv6 any any\nend")), ipv4ACL, "EDGE")
	if err != nil || absent != nil {
		t.Fatalf("absent ACL = %+v, %v", absent, err)
	}
	if _, _, err := nativeIP(configtest.Parse(t, nativeFixture("ip access-list standard EDGE\n sequence 10 permit any\nend")), ipv4ACL, "EDGE"); err == nil {
		t.Fatal("IPv4 namespace collision accepted")
	}
}

func TestNativeIPv6Protocol(t *testing.T) {
	current, _, err := nativeIP(configtest.Parse(t, nativeFixture(`ipv6 access-list EDGE
...
func TestNativeServices(t *testing.T) {
	current, _, err := nativeIP(configtest.Parse(t, nativeFixture(`ip access-list extended EDGE
...
func TestNativeProtocolNames(t *testing.T) {
	for name, number := range map[string]int64{"ipencap": 4, "ipip": 94, "ahp": 51, "ipv6-icmp": 58, "st2": 5, "st": 118, "divert": 254, "253": 253} {
		t.Run(name, func(t *testing.T) {
			current, _, err := nativeIP(configtest.Parse(t, nativeFixture("ip access-list extended 100\n sequence 10 permit "+name+" any any\nend")), ipv4ACL, "100")
			if err != nil {
				t.Fatal(err)
			}
...
func TestNativeIPv6Logging(t *testing.T) {
 ...
 sequence 20 deny ipv6 any any
	...
	if _, _, err := nativeIP(configtest.Parse(t, nativeFixture("ipv6 access-list EDGE\n sequence 10 deny ipv6 any any log log\nend")), ipv6ACL, "EDGE"); err == nil {
