...
func TestGlobalConfiguration(t *testing.T) {
	current, _, err := nativeGlobal(`ver 09.0.10kT213
!
authentication
...
func TestGlobalDefaults(t *testing.T) {
	for _, native := range []string{
		...
		current, _, err := nativeGlobal(native)
		if err != nil {
			t.Fatal(err)
		}
...
func TestInvalidGlobalConfiguration(t *testing.T) {
	for name, native := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			if _, _, err := nativeGlobal("authentication\n" + native + "end\n"); err == nil {
...
func TestGlobalOwnership(t *testing.T) {
	_, unowned, err := nativeGlobal("ver 09.0.10kT213\nauthentication\n auth-default-vlan 3055\n dot1x enable\n dot1x guest-vlan 3058\n dot1x enable ethe 1/1/10\n reauth-period 120\ninterface ethernet 1/1/9\n port-name neighbor\nend")
	if err != nil {
		t.Fatal(err)
	}
