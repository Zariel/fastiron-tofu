...
func TestVE(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if state.Exists != tc.exists || state.PortName != tc.portName || state.HasChildren != tc.children {
