...
func TestNativeAAAServer(t *testing.T) {
	for _, tc := range []struct {
		...
		{"radius defaults", "radius-server host 192.0.2.53", AAAServer{Kind: "radius", Address: "192.0.2.53", AuthPort: 1812, AcctPort: 1813, Purpose: "default"}, false},
		{"radius configured", "radius-server host 192.0.2.53 auth-port 1912 acct-port 1913 accounting-only key 2 opaque", AAAServer{Kind: "radius", Address: "192.0.2.53", AuthPort: 1912, AcctPort: 1913, Purpose: "accounting-only"}, true},
		{"tacacs defaults", "tacacs-server host 192.0.2.53", AAAServer{Kind: "tacacs", Address: "192.0.2.53", AuthPort: 49, Purpose: "default"}, false},
		{"tacacs configured", "tacacs-server host 192.0.2.53  auth-port 2112 authorization-only key 2 opaque", AAAServer{Kind: "tacacs", Address: "192.0.2.53", AuthPort: 2112, Purpose: "authorization-only"}, true},
	} {
...
func TestNativeAAAOwnership(t *testing.T) {
	desired := AAAServer{Kind: "radius", Address: "192.0.2.53"}
...
func TestAAAWritesRequireOptIn(t *testing.T) {
	d := &Device{}
	s := AAAServer{Kind: "radius", Address: "192.0.2.53", AuthPort: 1812, AcctPort: 1813, Purpose: "default"}
	for _, present := range []bool{true, false} {
		if _, err := d.ApplyAAAServer(context.Background(), s, nil, present); err == nil || !strings.Contains(err.Error(), "allow_aaa_changes") {
...
func TestAAAKeyValidation(t *testing.T) {
	d := &Device{config: Config{AllowAAAChanges: true}, discoveryGate: make(chan struct{}, 1)}
	for _, kind := range []string{"radius", "tacacs"} {
		s := AAAServer{Kind: kind, Address: "192.0.2.53", AuthPort: 49, Purpose: "default"}
		if kind == "radius" {
		...
		for _, key := range []string{"", "test key", "test\nkey", "test\x1bkey", strings.Repeat("x", limit+1)} {
			if _, err := d.ApplyAAAServer(context.Background(), s, &key, true); err == nil || !strings.Contains(err.Error(), "AAA key") {
...
func TestNativeAAAOrder(t *testing.T) {
	s := AAAServer{Kind: "radius", Address: "192.0.2.53"}
...
func TestKeylessTACACS(t *testing.T) {
	d := &Device{config: Config{AllowAAAChanges: true}, discoveryGate: make(chan struct{}, 1)}
	s := AAAServer{Kind: "tacacs", Address: "192.0.2.53", AuthPort: 49, Purpose: "default"}
	if _, err := d.ApplyAAAServer(context.Background(), s, nil, true); err == nil || !strings.Contains(err.Error(), "keyless configuration") {
